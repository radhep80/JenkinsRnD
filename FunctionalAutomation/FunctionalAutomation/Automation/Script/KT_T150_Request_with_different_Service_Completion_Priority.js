...
var serviceCompletionPriority = dataDriver.getValue("ServiceCompletionPriority");

function KT_T150_Register_a_request_with_a_different_value_of_Service_Completion_Priority() {
  try {
    aqTestCase.Begin("KT_T150_Register_a_request_with_a_different_value_of_Service_Completion_Priority");
    ...
    PG_Patient_Request.clickOnServiceCompletionPriority();

    //<Step 7>- Verify that "SELECT REQUEST PRIORTY TYPE" window appears. Select Request Priority as 'URGENT'. Click on Ok button
    PG_Patient_Request.selectServiceRequestPriorityType(serviceCompletionPriority);
